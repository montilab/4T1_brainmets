...
```{r data}
# Loading Count data
expression.path <- file.path(DPATH,"processed_data_old/Output/Expression/Yuhan_Gene_Expression.rds")
...
exp.data@colData<- exp.data@colData %>% 
  ...
  tidyr::separate(INDIVIDUAL_ID,"[C|R|S|B]",into=c("cell_type","replicate")) %>%
...
annot_col <- list(condition=c(C="gray",IR="pink", IS="orange", TGFB="purple"))
library(cba)
...
annot_col <- list(condition=c(C="gray",IR="pink",TGFB="purple"))
library(cba)
...
tiff(file=file.path(PATH, "results/dnmt_heatmap.tiff"), width=5, height=5, res=1200, units="in")
...
tiff(file=file.path(PATH, "results/h3_meth_genes_heatmap.tiff"), width=5, height=5, res=1200, units="in")
pheatmap(assays(dat_dnmt_flt)$rle_log2,
         annotation_col = annot,
         annotation_colors = annot_col,
         cluster_rows=hc_row,
...
# Cell Cycle genes 
```{r}
cell_cycle_genes <- read.delim(file.path(PATH, "data/cell_cycle_genes.txt"), header=FALSE) %>% pull(V1)

dat.filtered.genes <- unlist(rowData(dat.filtered)$mgi_symbol)
cell_cycle_genes_filter <- dat.filtered.genes %in% cell_cycle_genes
```

```{r}
dat_dnmt_flt <- 
  dat.filtered[cell_cycle_genes_filter,]
rownames(dat_dnmt_flt) <- unlist(rowData(dat_dnmt_flt)$mgi_symbol)
## column annotation
annot <- dat_dnmt_flt@colData %>% data.frame() %>% dplyr::select("condition")
annot_col <- list(condition=c(C="gray",IR="pink", IS="orange", TGFB="purple"))
...
tiff(file=file.path(PATH, "results/cell_cycle_genes_heatmap.tiff"), width=7, height=7, res=1200, units="in")
pheatmap(assays(dat_dnmt_flt)$rle_log2,
         annotation_col = annot,
         annotation_colors = annot_col,
         cluster_rows=hc_row,
...
```{r}
glucose_met_genes <- read.delim(file.path(PATH, "data/glucose_met_genes.txt"), header=FALSE) %>% pull(V1)

dat.filtered.genes <- unlist(rowData(dat.filtered)$mgi_symbol)
glucose_met_genes_filter <- dat.filtered.genes %in% glucose_met_genes
```

```{r}
dat_dnmt_flt <- 
  dat.filtered[glucose_met_genes_filter,]
rownames(dat_dnmt_flt) <- unlist(rowData(dat_dnmt_flt)$mgi_symbol)
## column annotation
annot <- dat_dnmt_flt@colData %>% data.frame() %>% dplyr::select("condition")
annot_col <- list(condition=c(C="gray",IR="pink", IS="orange", TGFB="purple"))
...
tiff(file=file.path(PATH, "results/glucose_met_genes_heatmap.tiff"), width=7, height=7, res=1200, units="in")
pheatmap(assays(dat_dnmt_flt)$rle_log2,
         annotation_col = annot,
         annotation_colors = annot_col,
         cluster_rows=hc_row,
...
# Fatty Acid genes 
```{r}
fatty_acid_genes <- read.delim(file.path(PATH, "data/fatty_acid_genes.txt"), header=FALSE) %>% pull(V1)

dat.filtered.genes <- unlist(rowData(dat.filtered)$mgi_symbol)
fatty_acid_genes_filter <- dat.filtered.genes %in% fatty_acid_genes
```

```{r}
dat_dnmt_flt <- 
  dat.filtered[fatty_acid_genes_filter,]
rownames(dat_dnmt_flt) <- unlist(rowData(dat_dnmt_flt)$mgi_symbol)
## column annotation
annot <- dat_dnmt_flt@colData %>% data.frame() %>% dplyr::select("condition")
annot_col <- list(condition=c(C="gray",IR="pink", IS="orange", TGFB="purple"))
...
tiff(file=file.path(PATH, "results/fatty_acid_genes_heatmap.tiff"), width=7, height=7, res=1200, units="in")
pheatmap(assays(dat_dnmt_flt)$rle_log2,
         annotation_col = annot,
         annotation_colors = annot_col,
         cluster_rows=hc_row,
...
# Save data
```{r}
saveRDS(dat.filtered,file=file.path(PATH,"data/4T1_sExp.rds"))
saveRDS(dat_pool,file=file.path(PATH,"data/4T1_control_IS_pool_sExp.rds"))
```
